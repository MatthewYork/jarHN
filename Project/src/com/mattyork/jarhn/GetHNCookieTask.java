...
public class GetHNCookieTask extends AsyncTask<Void, Void, Void> {
	...
	protected Void doInBackground(Void... arg0) {
		...
		try {
			//Make the request
			HttpResponse response = HNManager.getInstance().Service.client.execute(httpGet);
			
			//Get cookie
			List<Cookie> cookies = HNManager.getInstance().Service.client.getCookieStore().getCookies();
			if (cookies.size() > 0) {
